...
void HtmlButtons(WiFiClient cl){
  ...
  cl.println("<input type=\"button\" value=\"PIN 0 OFF\" onclick=location.href=\"/?PIN0OFF\">");
  cl.println("<input type=\"button\" value=\"PIN 0 ON\" onclick=location.href=\"/?PIN0ON\">");
}

void Commands(){
  //Switch led
  if (request.indexOf("/?PIN0ON") > -1)
  {
    Serial.println("LED value: ON");
    digitalWrite(D0, HIGH);
  }
  else if (request.indexOf("/?PIN0OFF") > -1)
  {
    Serial.println("LED value: OFF");
    digitalWrite(D0, LOW);
  }
}
